...
import { useScorecard } from "@/hooks/useScorecardService"
...
export default function ViewScorecardPage() {
  ...
  const { data: scorecard, isLoading: scorecardLoading } = useScorecard(scorecardId)
  const { data: course, isLoading: courseLoading } = useCourse(scorecard?.courseId)
  ...
  const getScoreBadgeVariant = (scoreToPar: number) => {
    if (scoreToPar < 0) return "default"
    if (scoreToPar === 0) return "secondary"
    return "destructive"
  }
