...
interface PlayersViewProps {
  initialPlayers: PlayerListGetDTO[];
}

export function PlayersView({ initialPlayers }: PlayersViewProps) {
  const router = useRouter()
  const [viewMode, setViewMode] = useState<"table" | "grid">("table")
  const [deletingPlayerId, setDeletingPlayerId] = useState<number | null>(null)
  
  // TanStack Query hooks with initial data
  const { data: playersData, isLoading, error, refetch } = usePlayers(
    { pageNumber: 1, pageSize: 100 },
    {
      initialData: {
        ...
        pageSize: 100,
        totalPages: 1,
        totalCount: initialPlayers.length
      } as PaginationResponse<PlayerListGetDTO>
    }
  )
  ...
  const confirmDelete = async () => {
    ...
    await deletePlayerMutation.mutateAsync(deletingPlayerId)
